...
class Predictor:
    def __init__(self, model=None, tokenizer=None, seq_len=3, num_gen_words=3):
        if model:
            self.model = model
        else:
            self.model = load_model(os.path.join('checkpoints', 'model.h5'))

        if tokenizer:
            self.tokenizer = tokenizer
        else:
            self.tokenizer = load(
                open(os.path.join('pickle_objects', 'tokenizer_model4'), 'rb'))
    ...
    def textGenerator(self, seed_text):
        ...
        for i in range(self.num_gen_words):
            ...
            pad_encoded = pad_sequences(
                [encoded_text], maxlen=self.seq_len, truncating='pre')
